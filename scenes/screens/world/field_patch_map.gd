...
const DEBUG_FONT := preload("res://assets/fonts/Pixeled.ttf")
...
var draw_debug_tile_grid := false
var draw_debug_tile_labels := false
...
func generate(condition: FieldCondition, seed_value: int, source_environment_map: EnvironmentMap = null) -> void:
	...
	_print_patch_generation_debug()
	queue_redraw()

func get_patch_at(global_world_position: Vector2) -> FieldCondition.PatchSet:
...
func _pick_patch_at(world_position: Vector2, seed_value: int) -> FieldCondition.PatchSet:
...
func _draw() -> void:
	...
	if draw_debug_tile_grid:
		_draw_debug_tile_grid()

func _draw_debug_tile_grid() -> void:
	for y in rows:
		for x in columns:
			var patch: int = terrain_grid[y][x]
			if patch == FieldCondition.PatchSet.NONE:
			...
			var rect := Rect2(FIELD_RECT.position + Vector2(x, y) * TILE_SIZE, TILE_SIZE)
			var color := EnvironmentMap.get_patch_color(patch)
			var border_color := Color(color.r, color.g, color.b, 0.95)
			draw_rect(rect, Color(color.r, color.g, color.b, 0.10), true)
			draw_rect(rect, border_color, false, 1.0)
			if draw_debug_tile_labels:
				draw_string(
					DEBUG_FONT,
					rect.position + Vector2(3.0, 9.0),
					_get_patch_debug_label(patch),
...
func _print_patch_generation_debug() -> void:
	var total_cells := columns * rows
	var counts := {}
	for y in rows:
		for x in columns:
			var patch: int = terrain_grid[y][x]
			counts[patch] = int(counts.get(patch, 0)) + 1

	print("[FieldPatchMap] grid=%dx%d tile=%s seed=%d" % [columns, rows, str(TILE_SIZE), generation_seed])
	var printed_special_patch := false
	for patch in counts.keys():
		if int(patch) == FieldCondition.PatchSet.NONE:
			continue
		printed_special_patch = true
		var count := int(counts[patch])
		var percent := float(count) / maxf(float(total_cells), 1.0) * 100.0
		var modifier := FieldCondition.get_patch_modifiers(int(patch))
		print("[FieldPatchMap] %s cells=%d %.1f%% modifiers player_speed=%.2f acceleration=%.2f stopping_friction=%.2f slip=%.2f ball_ground_friction=%.2f" % [
			FieldCondition.PatchSet.keys()[int(patch)],
			count,
			percent,
			modifier["player_speed"],
			modifier["acceleration"],
			modifier["stopping_friction"],
			modifier["slip"],
			modifier["ball_ground_friction"],
		])
		print("[FieldPatchMap] %s actual_on_tile player_speed=%.2f acceleration=%.2f stopping_friction=%.2f slip=%.2f ball_ground_friction=%.2f" % [
			FieldCondition.PatchSet.keys()[int(patch)],
			field_condition.get_player_speed_multiplier() * modifier["player_speed"],
			field_condition.get_acceleration_multiplier() * modifier["acceleration"],
			field_condition.get_stopping_friction_multiplier() * modifier["stopping_friction"],
			field_condition.get_slip_chance_per_second() + modifier["slip"],
			field_condition.get_ball_ground_friction_multiplier() * modifier["ball_ground_friction"],
		])
	if not printed_special_patch:
		print("[FieldPatchMap] no special patch cells generated for this PatchSet")

func _get_patch_debug_label(patch: int) -> String:
	match patch:
